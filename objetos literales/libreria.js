...
function agregarLibro() {
    ...
    if (titulo && !isNaN(publicacion) && autor) {//eso lo que hace es ver si los datos estan vacios
        let libro = {//bien
            portada: portada,
            titulo: titulo,
            autor: autor,
...
function devolverLibro(index){
    ...
    let confirmacion = confirm("¿Está seguro de la devolución?")
    
    if (confirmacion){
        libreria[index].prestado = ""
        libreria[index].disponible = "Si"
        localStorage.setItem('libros', JSON.stringify(libreria));
        mostrarLibros()   
    }
...
function prestarLibro(index){
    ...
    let nombre = prompt("Prestado a: ")
    if (nombre){
        libreria[index].prestado = nombre
        libreria[index].disponible = "No"
        localStorage.setItem('libros', JSON.stringify(libreria));
        mostrarLibros()   
    }
...
function eliminarLibro(index) {
    ...
    if (/* index >= 0 && index < libreria.length */confirm("¿Está seguro de eliminar el libro?")){
...
function mostrarLibrosOrdenados(criterioOrden) {
    ...
    localStorage.setItem("ordenamiento", criterioOrden);
}

let orden = localStorage.getItem("ordenamiento");
mostrarLibrosOrdenados(orden);
